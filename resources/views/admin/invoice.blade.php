...
<body>
    <header>
        ...
        <span><img alt="" src="{{ asset('image/logo.jpg') }}" onerror="this.style.display='none'"></span>
    </header>
    <article>
        <h1>Recipient</h1>
        <address>
        ...
        <table class="inventory">
            ...
            <tbody>
                <tr>
                    ...
                    <td><span>$</span><span>{{ $roomRate }}</span></td>
                    <td><span>{{ $payment->NoofRoom }} </span></td>
                    <td><span>$</span><span>{{ number_format($payment->roomtotal, 2) }}</span></td>
                </tr>
                <tr>
                    <td><span>{{ $payment->Bed }} Bed </span></td>
                    <td><span>{{ $payment->noofdays }}</span></td>
                    <td><span>$</span><span>{{ $bedRate }}</span></td>
                    <td><span>{{ $payment->NoofRoom }} </span></td>
                    <td><span>$</span><span>{{ number_format($payment->bedtotal, 2) }}</span></td>
                </tr>
                <tr>
                    <td><span>{{ $payment->meal }} </span></td>
                    <td><span>{{ $payment->noofdays }}</span></td>
                    <td><span>$</span><span>{{ $mealRate }}</span></td>
                    <td><span>{{ $payment->NoofRoom }} </span></td>
                    <td><span>$</span><span>{{ number_format($payment->mealtotal, 2) }}</span></td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="balance">
            <tr>
                ...
                <td><span>$</span><span>{{ number_format($payment->subtotal, 2) }}</span></td>
            </tr>
            <tr>
                <th><span>Service Tax (10%)</span></th>
                <td><span>$</span><span>{{ number_format($payment->tax_amount, 2) }}</span></td>
            </tr>
            <tr>
                <th><span>Discount</span></th>
                <td><span>$</span><span>{{ number_format($payment->discount, 2) }}</span></td>
            </tr>
            <tr>
                <th><span>Grand Total</span></th>
                <td><span>$</span><span>{{ number_format($payment->grand_total, 2) }}</span></td>
            </tr>
            <tr>
                <th><span>Amount Paid ({{ $payment->method ?? '—' }})</span></th>
                <td><span>$</span><span>{{ number_format($payment->amount_paid, 2) }}</span></td>
            </tr>
            <tr>
                <th><span>Balance Due</span></th>
                <td><span>$</span><span>{{ number_format(max(0, $payment->balance), 2) }}</span></td>
            </tr>
            <tr>
