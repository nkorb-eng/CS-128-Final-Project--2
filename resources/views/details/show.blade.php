...
<style>
    /* Strips Quill editor borders when rendering HTML on the display page */
    .ql-editor-display {
...
<div style="padding-top: 80px; min-height: 85vh;" class="bg-light pb-5">
    ...
    <!-- ADMIN FLOATING EDIT CONTROLLER -->
    @if (session('is_admin'))
      <div class="shadow-lg p-3 rounded-4 border border-secondary" 
        ...
        <div class="d-flex align-items-center justify-content-between mb-2">
          ...
          <small class="text-warning fw-bold"></i> Page Editor</small>
        </div>
        <p class="small text-white-50 mb-3" style="font-size: 0.85rem;">
            Edit name, image & description for this {{ $detail->category }}.
        </p>
          ...
          <div class="modal-content">
            ...
            <form action="{{ route('admin.detail.update', $detail->id) }}" method="POST" enctype="multipart/form-data" id="editDetailForm">
              ...
              <div class="modal-body">
                <!-- ROOM / FACILITY NAME ONLY -->
                <div class="mb-3">
                  <label class="form-label fw-bold">
                    {{ $detail->category === 'room' ? 'Room Name' : 'Facility Name' }}
                  </label>
                  <input type="text" name="title" class="form-control" value="{{ $detail->title }}" required>
                  <small class="text-muted">Changes the display name shown to guests.</small>
                </div>

                <div class="mb-3">
                  <label class="form-label fw-bold">Header / Banner Image</label>
                  <input type="file" name="image" class="form-control" accept="image/*">
                  <small class="text-muted">Upload a new photo to replace the current display image.</small>
                </div>
                ...
                <div class="mb-3">
                  ...
                  <!-- Hidden field to send Quill HTML to Controller -->
    ...
    <div class="container my-4">
        ...
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
            ...
            <div style="height: 380px; width: 100%; background-color: #1e293b; overflow: hidden; position: relative;">
                ...
                @endif
                <div style="position: absolute; bottom: 0; left: 0; right: 0; background: linear-gradient(transparent, rgba(0,0,0,0.8));" class="p-4 text-white">
            ...
            <div class="card-body p-4 p-md-5">
                <div class="row g-4">
                    <div class="col-lg-8">
                        ...
                        <!-- Rich Text Rendered HTML wrapped in Quill's .ql-editor class -->
                    ...
                    <div class="col-lg-4">
                        <div class="bg-light p-4 rounded-4 border border-secondary border-opacity-10">
                            @if ($detail->category === 'room')
                                <h5 class="fw-bold text-dark mb-3">Book Your Stay</h5>
                                <p class="text-muted small">Reserve {{ $detail->title }} for your upcoming stay at BlueBird Hotel.</p>
                                <a href="{{ route('room.book', ['type' => $detail->title]) }}" class="btn btn-primary btn-lg w-100 fw-bold shadow-sm">
                                    Book {{ $detail->title }}
                                </a>
                            @else
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        if (document.getElementById('quill-editor')) {
            ...
            // Copy Quill HTML content into hidden input on submit
